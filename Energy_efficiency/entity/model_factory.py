...
MetricInfoArtifact1 = namedtuple("MetricInfoArtifact1",
                                ["model_name", "model_object", "train_rmse","train_rmse1", "test_rmse","test_rmse1", "train_accuracy","train_accuracy1",
                                 "test_accuracy","test_accuracy1", "model_accuracy","model_accuracy1", "index_number"])
...
def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) -> MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            #logging.info(f"X_train in model factory \n {X_train}")
            y_train_pred = model.predict(X_train)
            y_test_pred = model.predict(X_test)

            #logging.info(f"train Prediction is \n {y_train_pred}")

            #Calculating r squared score on training and testing dataset
            if type(y_train_pred)==tuple:
                train_acc = r2_score(y_train, y_train_pred[0])
                test_acc = r2_score(y_test, y_test_pred[0])

                train_acc1 = r2_score(y_train, y_train_pred[1])
                test_acc1 = r2_score(y_test, y_test_pred[1])
                ...
                train_rmse1 = np.sqrt(mean_squared_error(y_train, y_train_pred[1]))
                test_rmse1 = np.sqrt(mean_squared_error(y_test, y_test_pred[1]))


                # Calculating harmonic mean of train_accuracy and test_accuracy
                model_accuracy = (2 * (train_acc * test_acc)) / (train_acc + test_acc)
                model_accuracy1 = (2 * (train_acc1 * test_acc1)) / (train_acc1 + test_acc1)

                diff_test_train_acc = abs(test_acc - train_acc)
                diff_test_train_acc1 = abs(test_acc1 - train_acc1)

                #logging all important metric
                logging.info(f"{'>>'*30} Score {'<<'*30}")
                logging.info(f"model1 Train Score\t\t Test Score\t\t Average Score")
                logging.info(f"       {train_acc}\t\t {test_acc}\t\t{model_accuracy}")

                logging.info(f"model2 Train Score\t\t Test Score\t\t Average Score")
                logging.info(f"       {train_acc1}\t\t {test_acc1}\t\t{model_accuracy1}")



                logging.info(f"{'>>'*30} Loss {'<<'*30}")
                logging.info(f"Diff test train accuracy for 1st model : [{diff_test_train_acc}].") 
                logging.info(f"Diff test train accuracy for 2nd model : [{diff_test_train_acc1}].") 
                logging.info(f"Train root mean squared error for 1st model : [{train_rmse}].")
                logging.info(f"Train root mean squared error for 2nd model : [{train_rmse1}].")
                logging.info(f"Test root mean squared error fro 1st model : [{test_rmse}].")
                logging.info(f"Test root mean squared error fro 2nd model : [{test_rmse1}].")
                ...
                if model_accuracy >= base_accuracy or model_accuracy1 >= base_accuracy and diff_test_train_acc < 0.05 or diff_test_train_acc1 < 0.05:
                    ...
                    metric_info_artifact = MetricInfoArtifact1(model_name=model_name,
                                                            ...
                                                            train_rmse1=train_rmse1,
                                                            test_rmse=test_rmse,
                                                            test_rmse1=test_rmse1,
...
class ModelFactory:
    ...
    def update_property_of_class(instance_ref:object, property_data: dict):
        '''
        Input:
            instance_ref:object of class returnde after calling class_for_name function
            property dict=model params dict
        Output:
            object with parameter set as property dict
            Example LinearRegression("param_name1": "value1",
                         "param_name2": "value2")
